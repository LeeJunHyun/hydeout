...
![CS294-02-09](/assets/img/CS294/CS294-02-09.png)
...
![CS294-02-10](/assets/img/CS294/CS294-02-10.png)
...
![CS294-02-11](/assets/img/CS294/CS294-02-11.png)
...
#### Distribution mismatch analysis with cost function
Cost function을 이용해서 [distribution mismatch(drift)](#drift)에 대한 분석을 해보자.

![CS294-02-12](/assets/img/CS294/CS294-02-12.png)

DAgger이 없다면 [distribution mismatch](#drift)에 의해 빨간 선처럼 어긋난 경로로 가게될 것이다. 이것이 얼마나 어긋났는지, 얼마나 잘못된 것인지 cost function을 통해 나타내보자. time 축을 time step $ T $로 정의하고, cost function은 아래와 같이 정의한다.
...
![CS294-02-13](/assets/img/CS294/CS294-02-13.png)

*즉, 줄위에서 사람이 떨어질 때만 cost가 발생한다.*

학습 알고리즘이 잘 돌아가서 모든 $ s \in \mathit{D_{train}} $에 대해 $ \pi_{\theta}(a \ne \pi^\star (s) \| s) \leq \epsilon $ 이라 가정하자. 학습데이터상의 모든 상태 $ s $ 에서 optimal과 다를(줄 위에서 떨어질) 확률이 $ \epsilon $이하인 것이다. 이때 total cost의 기댓값은 다음과 같다.

> $ \mathbb{E} [\  \mathsf{\underset{t}{\Sigma}}\  c(s_{t},a_{t})] \leq \epsilon T + (1-\epsilon)(\epsilon(T-1) + (1-\epsilon)(...)) $


$ T $ step을 간다고 했을 때, 첫 step에서 떨어질때의 cost 기댓값이 $ \epsilon T $이고, 두번째 step에서 떨어질 때의 cost 기댓값이 $ (1-\epsilon)\epsilon(T-1) $이고, 세번째 step에서 떨어질 때의 cost 기댓값이 $ (1-\epsilon)(1-\epsilon)\epsilon(T-2) $.... 이렇게 $ T $ step만큼의 항이 나온다. 그리고 각각의 항이 $ O(\epsilon T) $ (Order of $ T $)) 이기 때문에 전체 $ \mathbb{E} [\  \mathsf{\underset{t}{\Sigma}}\  c(s_{t},a_{t})] $ 는 $ O(\epsilon T^2) $가 된다.

이때 $ O(\epsilon T^2) $ 는 좋지 않다. $\epsilon$이 아무리 작다해도 경로가 길어질수록 cost가 지나치게 커지기 때문이다. 이것이 빨간 선이 초반에는 검정 선과 미세한 차이여도 종점에는 큰 차이가 나게 되는 [distribution mismatch](#drift)에 대한 cost 측면에서의 해석이다.
